...
class _InteractiveViewerState extends State<InteractiveViewer>
  ...
  void _receivedPointerSignal(PointerSignalEvent event) {
    if (event is PointerScrollEvent) {
      ...
      // New wheel/trackpad input supersedes the previous drag's momentum.
      // Otherwise _onAnimate overwrites this event's position on the next frame.
      _controller.stop();
      _animation?.removeListener(_onAnimate);
      _animation = null;
      _controller.reset();
      ...
      if (widget.scrollControls == InteractiveViewerScrollControls.scrollPans) {
        ...
        // Wheel deltas are viewport pixels; matrix translation uses scene units.
        // Convert both endpoints so zoom does not amplify scrolling distance.
        final Offset sceneTranslation =
            _transformationController!.toScene(event.localPosition + translation) -
